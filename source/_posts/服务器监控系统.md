...
type: null
updated: Sat, 25 Feb 2023 15:17:12 GMT
---
## Prometheus Server
...
```bash
docker run -it --log-opt max-size=10m --log-opt max-file=3 --name loki -config.file=/etc/loki/local-config.yaml -p 3100:3100 grafana/loki:2.2.0
# max-size 最大数值
# max-file 最大日志数
```
